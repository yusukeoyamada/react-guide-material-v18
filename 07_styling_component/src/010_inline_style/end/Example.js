/* POINT
  - 再利用性が低い
  - 疑似要素(::before, ::after)、疑似セレクタ(:hover, :active)や、メディアクエリ(@media)が使用できない
  ...
  */
  const style = {
    ...
    */
    ...
    // 「-」の部分(ケバブケース)は、削除して、大文字(キャメルケース)で繋げる。
    fontWeight: "bold",
    cursor: "pointer",

    /* POINT
      三項演算子を使用して isSelected が true の場合は 'pink'。false の場合は空文字('')を与えています。
      valueに空文字を与えた場合プロパティは適応されません。
    */
    backgroundColor: isSelected ? "pink" : "",

    /* POINT 
      直接記述することによって可読性が大きく低下するので、可読性が向上する方法を考えて実装してみよう
    */
  };
